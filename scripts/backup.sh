...
backup() {
    ...
    backupArr=(
        ...
        "$HOME/bitbucket/SL/.windsurf/rules.md"
        "$HOME/bitbucket/SL/.warp/rules.md"
        "$HOME/bitbucket/SL/.claude/rules.md"
    )

    for b in "${backupArr[@]}"; do
        # 7z a -bt -t7z -m0=lzma -mx=9 "$outputFile" -spf2 -p"$(pass backup)" "$b" -xr!.git -xr!node_modules >/dev/null
        7z a -bt -t7z -m0=lzma -mx=9 "$outputFile" -spf2 -p1 "$b" -xr!.git -xr!.venv -xr!node_modules -xr!themes -xr!*/tmp/* -xr!tmp >/dev/null
        echo "--> $b"
    done
