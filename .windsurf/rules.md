...
12. **Prefer explicit DTO/type aliases over utility soups**: If a service/controller already exports a clear type (e.g., `OrderStatusWidgets`, `FooResponseDto`), reuse it instead of verbose constructs like `Awaited<ReturnType<typeof service['method']>>`. Create/export concise aliases next to the source and import them wherever the contract is needed.
...
14. **Const/type parity**: When you describe a set via `as const`, give the constant and the exported type the same name so they behave like a pseudo-enum without extra aliases. Example: `export const ORDER_INCLUDES = { ... } as const; export type ORDER_INCLUDES = ValueOf<typeof ORDER_INCLUDES>;` — importing a single identifier then covers both the value and its type.
15. **Constants vs helpers**: Files under `*/constants/*` must export only literals, enum-like objects, and related types. Any logic/helpers (even simple functions such as `buildSomethingParam`) belong in `*/utils/*` or domain services. If a constant needs computation, move that function to `utils` and import it where needed—this keeps static data separate from operational logic.
16. **Membership checks**: When you need to determine whether a value belongs to a set (e.g., `OrderLabel`/`OrderStatus`), use `array.includes(...)` or `set.has(...)` with the collection stored in a constant. Avoid long `value === A || value === B || ...` chains—they are hard to read and extend.
17. **Filter parity**: Whenever you add or change a filter (query param, tab, dropdown option, or virtual values like `all`/`migrated-history`), keep the backend and frontend in sync. Update the DTOs, handlers, repositories, export services, and dependent widgets on the backend, plus the corresponding hooks/tab configs/query builders on the frontend. Before merging, manually test every entry point (UI list, export, and any integrations) to ensure the new value does not produce empty results or `NO_ORDERS_FOUND`-style errors caused by mismatched logic.
18. **Order constants reuse**: Do not hardcode literals like `'all'`, `'migrated-history'`, include lists, or timeouts in order-related code. Always import the shared constants from `@/entities/orders/model/constants` (e.g., `ALL_LIFECYCLE_STATUS`, `MIGRATED_HISTORY`, `ORDER_INCLUDES`, `ORDER_EXPORT_DEFAULT_INCLUDES`) and time helpers such as `MS_IN_MINUTE` from `@/shared/constants/time` so filters, API calls, and hooks stay aligned.
19. **Conditional spreads**: When conditionally adding properties to objects (query params, props, etc.), prefer `...(condition && { foo: value })` over ternaries that return `{}` (`...(condition ? { foo: value } : {})`). This keeps the spread concise and avoids constructing throwaway empty objects.
20. **Boolean conditional expressions**: For boolean props that depend on a condition, prefer logical AND (`&&`) over ternary operators that return `false`. Example: `showFilter={!isDisabled && filterEnabled}` instead of `showFilter={isDisabled ? false : filterEnabled}`. This makes the intent clearer and more concise.
21. **No reusable utilities in UI files**: Do not keep reusable helper functions (parsers/formatters/converters/validators) inside `ui/*` component files. Move them to the appropriate `model/utils/*` (or `entities/*/model/utils/*`) module and import them.
22. **Optional parameters over `T | undefined`**: If a parameter/prop is optional, prefer `param?: T` over `param: T | undefined`. Use `T | null | undefined` only when you explicitly need to accept both `null` and `undefined` as input values.
23. **Layering**: `model/*` (especially `model/utils/*`) must not import from `ui/*`. If a UI component currently exports constants needed elsewhere, move those constants into `entities/*/model/constants.ts` or `features/*/model/constants.ts` and import them from there.
24. **Type Checking Simplification**: Avoid unnecessary `typeof` checks for primitive types when the value can simply be truthy/falsy evaluated or coalesced using `??`. For example, instead of `typeof val === 'number' ? val : fallback`, use `val ?? fallback` (assuming `val` is properly typed as `number | undefined | null`). This keeps conditional logic cleaner and easier to read.
25. **Optional Boolean Property Checks**: For optional boolean properties (e.g., `hasUnsetBasePrice?: boolean`), prefer null checks (`if (property != null)`) over typeof checks (`if (typeof property === 'boolean')`). This is more concise and handles both `null` and `undefined` cases correctly.
26. **RBAC verification**: When invoking an API (frontend queries or backend route handlers), always confirm the endpoint’s role/permission requirements and ensure the calling context matches them. Never hook customer-facing screens to admin-only routes (and vice versa); if the same data is needed in multiple contexts, expose a dedicated endpoint or guard the request with feature flags/`enabled` checks so unauthorized roles never call it. Include RBAC validation in manual test plans before shipping.
27. **Data shaping belongs in hooks**: When we need deterministic ordering or derived fields (e.g., sorting Link Insert products by DR range), perform the transformation inside the shared data hook/query `select`, not inside consuming UI components. This keeps UI layers dumb, prevents duplicate logic, and guarantees every consumer receives the same normalized dataset.
